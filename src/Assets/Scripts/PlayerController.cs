...
public class PlayerController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        //操作を受けて動かす
        if (!_animationController.Update())//アニメーション中はキー入力を受け付けない
        {
            Control();
        }

        float anim_rate = _animationController.GetNormalized();
        _puyoControllers[0].SetPos(Interpole(_position, RotState.Invalid, _last_position, RotState.Invalid, anim_rate));
        _puyoControllers[1].SetPos(Interpole(_position, _rotate, _last_position, _last_rotate, anim_rate));
    }
